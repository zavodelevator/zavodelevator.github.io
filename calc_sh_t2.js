...
function prace_vue() {
  ...
  var dbgCalc = priceDebugFormula();
  renderPriceDebug(dbgCalc);
...
function renderPriceDebug(calc){
  if($("#price_debug_ex").length === 0){
    $(".price").after('<div id="price_debug_ex" class="alert alert-secondary" style="margin-top:10px;"></div>');
  }
  function fmt(n){ return String(Math.trunc(Number(n))).replace(/\B(?=(\d{3})+(?!\d))/g,' '); }
  var html = "<b>Формула ціноутворення (тимчасово)</b><br>"
    + calc.steps.join("<br>")
    + "<br>Транспортер: ₴ "+fmt(calc.transportPrice)
    + " | Привід: ₴ "+fmt(calc.enginePrice)
    + " | Разом: ₴ "+fmt(calc.total);
  $("#price_debug_ex").html(html);
}
